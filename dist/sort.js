...
(function ($) {
	...
	Page.prototype = {
		...
		service: {
			...
			coursePratice: false, //视频弹题
		},
		init: function () {
			$selectMenu("course_class_type");
			var $this = this;
			//随堂练习相关业务
			this.checkCoursePratice();
			this.coursePraticeEvent();
			this.init1500();
		...
		checkCoursePratice: function () {
			var _this = this;
			$.ajax({
				url: rootPath + "/classManage/loadFunctionSet",
				type: "post",
				data: {
					'functionCode': 'COURSE_VIDEOIN_FUNCTION'
				},
				dataType: "json",
				success: function (data) {
					data = data.cfs;
					if (data && (data.status == 1 || data.status == '1'))
						_this.service.coursePratice = true;
				}
			})
		},
				...
				success: function (jsonData) {
					$.each(jsonData, function (i) {
						...
						var html = '<li id="chapter_' + chapter.id + '" class="dis" sort="' + chapter.chapterOrder + '"><p class="father">' +
							...
							'<a href="javascript:;" title="' + (chapter.chapterName) + '">第' + getChapterName() + getCname() + '&nbsp;&nbsp;<b>' + (chapter.chapterName ? chapter.chapterName.length > 18 ? chapter.chapterName.substring(0, 18) + "..." : chapter.chapterName : "") + '</b></a>' +
							'<i class="iconfont">&#xe626;</i>' +
						...
						$.each(chapter.lectureAndTests, function (j) {
							...
							if (lecture.type == 1) {
								var sub_html = '<li id="lecture_' + lecture.id + '" class="class-son" draggable="true">' +
									'<a style="display: inline-block;width: 85%;"href="javascript:;"><span class="add-xiaojie"></span>第' +
								...
								var sub_html = '<li id="test_' + lecture.id + '" class="class-son" draggable="true">' +
									'<a style="display: inline-block;width: 85%;" href="javascript:;"><span class="iconfont add-ceyan">&#xe6d5;</span>测验' +
									'&nbsp;&nbsp;<b>' + (lecture.name ? lecture.name : "") + '</b></a>' +
									'<i class="iconfont4">&#xe630;</i>' +
									'<i class="iconfont">&#xe626;</i></li>';
								$(".t-c-l").find("#chapter_" + chapter.id + " .son").append(sub_html);
			...
			$.ajax({
				...
				success: function (jsonData) {
					if (jsonData.flag) {
						...
						var chapterName = jsonData.chapterName;
						chapterName = (chapterName ? chapterName.length > 18 ? chapterName.substring(0, 18) + "..." : chapterName : "");
						if (chapter.find(".father").find("a b").length > 0) {
							chapter.find(".father").find("a b").html(chapterName);
						} else {
							chapter.find(".father").find("a").html("<b>" + chapterName + "</b>");
						}
			...
			$.ajax({
				...
				success: function (jsonData) {
					var jsonDataCp,
						jsonDataCprs;
					if (jsonData.flag) {
						jsonDataCp = jsonData.cp;
						jsonDataCprs = jsonData.cprs;
						...
						$.msg("保存成功");
						if (jsonDataCp) {
							var _domCp = $('.course-practice').data();
							if (!_domCp.id) {
								_domCp.id = jsonDataCp.id;
							}
						}
						if (jsonDataCprs) {
							$tr = $('.course-practice').find('.c-p-r b');
							$tr.each(function (i) {
								var _domCpr = $(this).data();
								if (!_domCpr.id) {
									_domCpr.id = jsonDataCprs[i].id;
								}
							});
						}
					...
					if (dd && dd.length > 0) {
						if (data.testType == 0) {
							...
							for (var x = 0; x < dd.length; x++) {
								if (dd[x]) {
									subArr.push(parseInt(dd[x].subjectId));
								}
							}
							$.unique(subArr.sort());
							if (subArr.length > 1) {
		...
		delLecture: function (lecture) {
			var $this = this;
			var chapter = lecture.parents(".dis");
		...
		addLecture: function (lecture, callback) {
			...
			if (!para.lectureOrder) {
				para.lectureOrder = 1;
			}
			para.lectureOrder = para.lectureOrder + 1;
			$.ajax({
		...
		showChapter: function (chapter) {
			...
			if (chapter.attr("id")) {
				...
				var chapterDetail = '<div class="t-c-r-t chapter" value="' + chapter_id + '">' +
					...
					'<span class="c-content"><input class="chapterName" type="text" maxlength="70" value="" placeholder="章的名称"></span>' +
					'</p>' +
					'<p class="c">' +
				...
				var chapterDetail = '<div class="t-c-r-t chapter" value="">' +
					'<p class="c">' +
					'<span class="c-title">章的名称</span>' +
					'<span class="c-content"><input class="chapterName" maxlength="70" type="text" value="" placeholder="章的名称"></span>' +
					'</p>' +
					'<p class="c">' +
					'<span class="c-title"></span>' +
					'<span class="c-content">' +
		...
		showLecture: function (lecture) {
			...
			var lectureDetail = '<div class="project-part"> ' + '<h3 class="add-title">' +
				...
				'<span class="c-content">' +
				'<input class="lectureName" type="text" placeholder="输入名称">' +
				...
				(!_this.service.coursePratice || !_this.service.tikuService ? '' : '<span class="tab-right tab-course-practice none">随堂练习</span>') +
				'</div>' +
				'<div class="tab-item">' +
				...
				'   </p>' +
				'</div>' +
				(!_this.service.coursePratice || !_this.service.tikuService ? '' :
					'<div class="q-item three course-practice none st-item">' +
					'<p class="c">' +
					'<span class="c-title">是否启用：</span>' +
					'<span class="c-content">' +
					'<a href="javascript:;" class="">' +
					'<input type="radio" name="type-cp"  value="1" id="type-cp-open">' +
					'<label style="display:inline;" for="type-cp-open">启用</label>' +
					'</a>' +
					'<a href="javascript:;" class="">' +
					'<input type="radio" name="type-cp"  value="0" id="type-cp-close">' +
					'<label style="display:inline;" for="type-cp-close">禁用</label>' +
					'</a>' +
					'</span>' +
					'</p>' +
					'<p class="c cp-s-h">' +
					'<span class="c-title">视频时长：</span>' +
					'<span class="c-content">' +
					'<a href="javascript:;" class="">' +
					'<label style="display:inline;" class="video-time"></label>' +
					'</a>' +
					'</span>' +
					'</p>' +
					'<p class="c newContent cp-s-h">' +
					'<label for="">' +
					'每 <input type="text" name="" id="minutePer">分钟做一次题' +
					'</label>' +
					'<label for="">' +
					'每次做<input type="text" name="" id="topicNumPer">道题目' +
					'</label>' +
					'</p>' +
					'<div class="c newChooseC cp-s-h">' +
					'<span class="c-content">' +
					'<a href="javascript:;" class="q-btn btn btn-default choose-course-practice-tiku">选择章节</a>' +
					'</span>' +
					'</div>' +
					'<p class="c clear q-c cp-s-h">' +
					'<span class="c-title">章节列表：</span>' +
					'<span class="c-content c-p-r">' +
				...
				if (data.videoId) {
					...
					$.ajax({
						...
						success: function (jsonData) {
							...
							//给选择视频按钮存储数据
							$('.lecture .chooseVideo').data(null).data(jsonData);
							$('.lecture .video-time').html(jsonData.videoTime || '');
						}
					})
					if (data.freeFlag == 1) {
					...
					_this.showCoursePratice('TEACH_METHOD_VIDEO', lecture_id);
					$('.choose-course-practice-tiku').data('lecid', lecture_id);
				}

				if (data.fileId) {
					if (data.freeFlag == 1) {
						$('#free-sorce').attr('checked', true);
					}
					...
					$.ajax({
						...
						success: function (jsonData) {
							...
							/* if(jsonData.fileCategory == 'video')
                                $('.t-c-r').find('#type-paper').trigger('click');
							else if(jsonData.fileCategory == 'flash')
							...
							if (data.fileType == 'video')
								$('.t-c-r').find('#type-paper').trigger('click');
							else if (data.fileType == 'flash')
								$('.t-c-r').find('#type-paper2').trigger('click');
							else if (data.fileType == 'audio')
								$('.t-c-r').find('#type-paper3').trigger('click');
							else if (data.fileType == 'ppt')
								$('.t-c-r').find('#type-paper4').trigger('click');
							else if (data.fileType == 'docs')
								$('.t-c-r').find('#type-paper5').trigger('click');
						}
					})
					$('.file_id').val(data.fileId);
				} else {
					$('#type-cp-close').trigger('click');
				}
			}
			...
			$.ajax({
				...
				success: function (exercise) {
					if (exercise.tikuResourceType == "PAPER") {
						...
						$(".t-c-r").find(".n-list").html('<b><i class="list-item" title="' + exercise.paperName + '">' + exercise.paperName + '</i> <a class="del" eid="' + exercise.id + '"><strong>删除</strong></a></b>');
					} else {
		...
		showTest: function (test) {
			...
			var testDetail = '<div class="ceshi-part">' +
				...
				'<span class="c-content">' +
				'<input class="lectureName q-text" type="text" placeholder="输入名称"></span>' +
				'</p>' +
				'<p class="c">' +
				'<span class="c-title">达标分数：</span>' +
				'<span class="c-content">' +
				'<input type="text" class="video_name q-text1" placeholder="请填写整数且范围为：0-100">' +
				'</span>' +
				'</p>' +
		...
		searchPapers: function (page) {
			...
			$.ajax({
				...
				success: function (jsonData) {
						...
						$.each(jsonData.data, function (i, data) {
							$tab.find("#data_table_2").find("tbody")
								.append('<tr id="' + data.id + '">' +
									...
									'<td style="width:25%;"  title="' + data.categoryName + '"><div style="width: 200px;height: 16px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden">' + (data.categoryName ? data.categoryName : "") + '</div></td>' +
									'<td style="width:25%;" title="' + data.subjectName + '">' + (data.subjectName ? data.subjectName : "") + '</td>' +
									'<td style="width:25%;" title="' + data.dictName + '">' + (data.dictName ? data.dictName : "") + '</td>' +
									'</tr>');
					...
					$tab.css("marginTop", (-$tab.height() / 2) + "px")
				}
			})
		},
		//检索视频
						...
						$.each(jsonData.data, function (i, data) {
							$tab.find("#data_table").find("tbody")
								.append('<tr id="' + data.id + '" videoCcId="' + data.videoCcId + '" has-data="true" videoStatus="' + data.videoStatus + '">' +
									...
									'</tr>');
							$tab.find("#data_table").find("tbody").find("tr:last").data("video", data);
						})
						$tab.find(".pagination").pagination(jsonData.rowCount, {
							next_text: "下一页",
							prev_text: "上一页",
		...
		searchResource: function (page) {
			...
			$.ajax({
				...
				success: function (jsonData) {
					...
					$('.q-btn-primary2').attr('disabled', false);
				},
				error: function (resp, err, msg) {
					console.log(resp);
					alert("检索异常");
					...
					if (data == null || data.length == 0) {
						...
						$.each(data, function (i, item) {
							...
							if (name.substring(name.lastIndexOf(".") + 1) == "doc" || name.substring(name.lastIndexOf(".") + 1) == "docx") {
								html += '<b><i class="list-item" title="' + item.name + '">' + item.name + '</i><a href="javascript:void(0);" class="delresource"  ids="' + item.id + '"><strong>删除</strong></a></b>';
		...
		initBase: function () {
			...
			$(".teacher-btns").on("click.btn.page", ".btn-add-class", function () {
				var chapter = '<li class="dis none" draggable="true"><p class="father">' +
					'<i class="iconfont4">&#xe630;</i>' +
					'<i class="iconfont2 open">&#xe62f;</i>' +
			...
			$(".t-c-r").on("click.btn.right", ".save", function () {
				...
				if ($(this).parents(".chapter").length) {
					var chapter_name = $(this).parents(".t-c-r").find(".chapterName").val();
					if (!chapter_name) {
						$.msg('请输入章节名称');
						return;
					}
					...
					for (var i = 0; i < chapter_name.length; i++) {
						if (chapter_name.charCodeAt(i) > 255) //如果是汉字，则字符串长度加2
							str_len += 2;
						else
							str_len++;
					}
					if (str_len > 70) {
						$.msg("章名称最多输入70个字符（一个汉字占2位）");
						return;
					}
				...
				if ($(this).parents(".lecture").length) { //视频
					...
					if (lecture.find('#type-paper').is(':checked')) {
						...
						var cp_cprs = $this.getCoursePratice();
						if (!cp_cprs) {
							return;
						}
						data = $.extend(data, cp_cprs);
					} else { //其他类型
					...
					var chapter_len = 0;
					var chapter_name = lecture.find(".lectureName").val();
					for (var i = 0; i < chapter_name.length; i++) {
						if (chapter_name.charCodeAt(i) > 255) //如果是汉字，则字符串长度加2
							chapter_len += 2;
						else
			...
			$('.t-c-r').on('click', '#saveCeyan', function () {
				...
				if (flag) {
					data.testType = 1;
					if (isNaN(data.testScore) || data.testScore < 0) {
						$.msg('请输入有效的达标分数(正整数)');
						$('.q-text1').val('').focus();
						return;
					}
				...
				var test_len = 0;
				for (var i = 0; i < data.testName.length; i++) {
					if (data.testName.charCodeAt(i) > 255) //如果是汉字，则字符串长度加2
						test_len += 2;
					else
						test_len++;
				}
				if (test_len > 80) {
					$.msg("测验名称最多输入80个字符（一个汉字占2位）");
					return;
				}
			...
			$(".closeUpload,.closeBtn").click(function () {
				$("#class-resource").hide();
				$(".loading-bg").hide();
			});
			...
			//保存资料
			$(".btn-ok,.saveUpload").on('click', function () {
				$(".loading-bg").show();
				var fielIds = new Array();
				var oneItem = $("#itemOneId").val();
				var twoItem = $("#itemSecondId").val();
				var classid = $("#classTypeId").val();
				//类型code
				var resource = $("#sourceType").val();
				//类型name
				var resName = $.trim($("#sourceType").find("option:selected").text());
				var classTypeName = $("#classTypeName").val();
				var lectureId = $("#doctype").attr("lecid");
				var courseSourceUl = $(".courseSourceUl li").length;
				if (!(courseSourceUl > 0)) {
					$.msg("请先上传文档");
					return false;
				}
				for (var i = 0; i < courseSourceUl; i++) {
					fielIds[i] = $(".courseSourceUl li").eq(i).attr("fileid");
				}
			...
			$('.t-c-r').on('click', 'a.delrel', function () {
				var $that = $(this);
				$.confirm("您是否确认取消绑定此章节？", function (b) {
					if (b) {
						$that.parent().remove();
			...
			$('.t-c-r').on('click', '.del', function () {
				var $that = $(this);
				$.confirm("您是否确认取消绑定此试卷？", function (b) {
					if (b) {
						$that.parent().remove();
						var testId = $('.t-c-r').find('.test').attr('value');
						var items = $(".t-c-r").find("#courseList" + testId).data("exercises");
						...
						if (items && items.length > 0) {
							for (var x = 0; x < items.length; x++) {
								...
								if (id2 && items[x] && items[x].paperId && id2 == items[x].paperId) {
			...
			$('.t-c-r').on('click', '.q-type-choice', function () {
				...
				$("#free-sorce").show();
				$("#open-veido").hide();
				$('.tab-course-practice,.course-practice').hide();
				if ($('.tab-course-practice').hasClass('active')) {
					$('.tab-list').find('span').eq(0).trigger('click');
				}
				...
				$.ajax({
					...
					success: function (exercise) {
						if (exercise && exercise.tikuResourceType == "CHAPTER") {
							$(".t-c-r").find(".n-list").html('<tr><td width="30%">' + (exercise.chapterName ? exercise.chapterName : "") + '</td><td width="30%">' + (exercise.sectionName ? exercise.sectionName : "") + '</td><td width="30%"><a href="javascript:;" eid="' + exercise.id + '">删除</a></td></tr>');
						}
						$(".t-c-r").find(".n-list").data("exercise", exercise);
					}
				...
				$.ajax({
					...
					success: function (jsonData) {
						if (jsonData && jsonData.length > 0) {
							...
							$.each(jsonData, function (i, data) {
								$(".t-c-r").find("#courseList" + testId).append('<b><i class="list-item" title="' + data.paperName + '">' + data.paperName + '</i> <a class="del" mark1="' + data.paperId + '"><strong>删除</strong></a></b>');
							})
				...
				$.ajax({
					...
					success: function (jsonData) {
						if (jsonData && jsonData.length > 0) {
							...
							$.each(jsonData, function (i, data) {
								$(".t-c-r").find("#courseList" + testId).append('<b><i class="list-item" title="' + data.chapterName + '-' + data.sectionName + '">' + data.chapterName + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + data.sectionName + '</i><a class="delrel" mark="' + data.id + '" delIndex = "' + i + '"href="javascript:;"><strong>删除<strong></a></b>');
							})
							$(".t-c-r").find("#courseList" + testId).data('exercises', jsonData);
						} else {
							$('.t-c-r').find('#ziliaotitle').text('');
		...
		init800: function () {
			...
			//初始化一学科小类
			$.ajax({
				url: rootPath + "/video/openShareData",
				type: "post",
				async: false,
				success: function (data) {
					shareStatus = data;
			...
			if (shareStatus) {
				$.ajax({
					url: rootPath + "/video/getOneItemData",
					type: "post",
					success: function (data) {
						var tab1one = $(".tab1").find("#choose_itemOne");
						tab1one.append("<option value='0'>请选择学科</option>");
						$.each(data, function (i, item) {
							tab1one.append('<option value="' + item.id + '">' + item.itemName + '</option>');
						});
					}
				});
				$(".tab1").find("#choose_itemOne").change(function () {
					$.ajax({
						...
						success: function (data) {
							...
							$(".tab1").find("#choose_itemSecond").html("").html(data);
						}
					});
				});
				...
				success: function (jsonData) {
					...
					$.each(jsonData, function (i, data) {
						if (data) {
							datas.push({
								id: data.id,
								text: (data.tagName ? data.tagName : "")
							});
						}
					});
					$(".tag").each(function () {
			...
			$(".w800").on("click.row.model", ".term-list tr[has-data]", function () {
				...
				.on("click.row.model", ".term-list tr[has-data]", function () {
					...
					//给选择视频按钮存储数据
					$('.lecture .chooseVideo').data(null).data($(this).data('video'));
					$('.lecture .video-time').html($(this).data('video').videoTime || '');
					$(".video_name").val($(this).children().eq(0).attr("title"));
				...
				.on("click", ".btn-add-url", function () {
					...
					if (httpTitle == "v.youku.com") {
						...
						outUrl = o1 + "," + o2;
						$("#videourl").data("outUrl", outUrl);
					} else if (httpTitle.endsWith('gensee.com')) { //http://yunduoketang.gensee.com/training/site/v/04435895
						storageType = 'VIDEO_STORAGE_TYPE_ZS';
						outUrl = totalUrl.split('gensee.com/training/site/v/')[1]; //04435895
						if (outUrl) {
							$("#videourl").data("outUrl", outUrl);
							$.msg('已添加外部链接');
							return;
				...
				.on("click.btn.save", ".tab2 .btn-s", function () {
					...
					var totalUrl = url = $(".add-div").find("#videourl").val();
					var outUrl = $("#videourl").val();
					...
					var httpTitle = url.substring(0, url.indexOf("/"));
					var videoType = httpTitle;
					if (storageType != 'VIDEO_STORAGE_TYPE_OTHER') {
						videoType = totalUrl;
					}
					//				var storageType="";
					//				if(httpTitle == "v.youku.com"){
		...
		init900: function () {
			...
			$(".w900").on("change", "#choose_tiku", function () {
				...
				.on("click.tr.choose", "#data_table_2 tr", function () {
					...
					$(".t-c-r").find(".n-list").html('').append('<b><i class="list-item" title="' + d.paperName + '">' + d.paperName + '</i> <a class="del" ><strong>删除</strong></a></b>');
		...
		init1300: function () {
			...
			$(".w1400").on("change", "#choose_tiku_3", function () {
				...
				.on("click.btn", ".btn", function () {
					...
					$(".t-c-r").find("#courseList" + data.testId).append('<b><i class="list-item" title="' + data.chapterName + '-' + data.sectionName + '">' + data.chapterName + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + data.sectionName + '</i> <a href="javascript:;" class="delrel" delIndex = ' + i + '><strong>删除</strong></b>');
					++i;
				...
				.on("click.tr.choose", "#data_table_2 tr", function () {
					...
					if (items && items.length > 0) {
						...
						if (flag1) {
							...
							$(".t-c-r").find("#courseList" + data.testId).append('<b><i class="list-item" title="' + d.paperName + '">' + d.paperName + '</i> <a class="del" mark1="' + d.id + '"><strong>删除</strong></a></b>');
						}
					} else {
						items.push(data);
						if ($('.t-c-r').find('#ziliaotitle').text() == '') {
		 ...
		 * 随堂练习相关
		 */
		showCoursePratice: function (resourceType, lectureId) {
			var dataInfo = {};
			dataInfo.resourceId = lectureId;
			dataInfo.resourceType = resourceType;
			$.ajax({
				url: rootPath + '/coursePractice/showCoursePratice',
				type: 'post',
				data: dataInfo,
				dataType: 'json',
				success: function (jsonData) {
					var coursePractice = jsonData.coursePracticeVo,
						coursePracticeRanges,
						cpr,
						_html = '';

					//练习设置
					$('.course-practice').data(coursePractice);
					if (!coursePractice || !coursePractice.status) {
						$('#type-cp-close').trigger('click');
					} else {
						$('#type-cp-open').trigger('click');
					}
					if (!coursePractice)
						return;

					coursePracticeRanges = coursePractice.coursePracticeRanges;

					$('#minutePer').val(coursePractice.minutePer || '');
					$('#topicNumPer').val(coursePractice.topicNumPer || '');

					for (var i = 0; i < coursePracticeRanges.length; i++) {
						cpr = coursePracticeRanges[i];
						_html = '<b>' +
							'<i class="list-item" title="' + cpr.tikuChapterName + '">' + cpr.tikuChapterName + '</i>' +
							'<i class="list-item" title="' + cpr.tikuSection + '">' + cpr.tikuSection + '</i>' +
							'<a class="del-cpr">' +
							'<strong>删除</strong>' +
							'</a>' +
							'</b>';
						$('.course-practice .c-p-r').append(_html);
						$('.course-practice .c-p-r').find('b:last').data(cpr);
					}
				}
			})
		},
		coursePraticeEvent: function () {
			//随堂练习的开启关闭
			$('.t-c-r').on('click', '.course-practice input[name="type-cp"]', function () {
				if ($(this).attr('id') == 'type-cp-open') {
					$('.cp-s-h').show();
				} else {
					$('.cp-s-h').hide();
				}
			...
			//删除随堂练习下的->抽题章节
			$('.t-c-r').on('click', '.course-practice .c-p-r .del-cpr', function () {
				var _data = $(this).parents('b').data(),
					$this = $(this);

				if (_data.id) {
					$.ajax({
						url: rootPath + '/coursePracticeRange/delete',
						type: 'post',
						data: {
							'id': _data.id
						},
			...
			//选择题库
			$('.t-c-r').on('click', '.course-practice .choose-course-practice-tiku', function () {
				$('.add-layer-bg').fadeIn(200, function () {
					$('.w1500').fadeIn(200);
				})
			});
		},
		getCoursePratice: function () {
			var _this = this;
			//开启随堂练习功能
			if (!_this.service.tikuService || !_this.service.coursePratice) {
				return {};
			}
			...
			var cp,
				...
				minutePer,
				$tr,
				cpr1, cpr2, csFlag = flag = true,
				cprs = [];

			cp = $('.course-practice').data();
			status = $('.course-practice input[name="type-cp"]:checked').val();
			topicNumPer = $('#topicNumPer').val();
			minutePer = $('#minutePer').val();
			...
			if (status == 1) {
				...
				if (!/^[1-9]\d*$/.test(minutePer)) {
					$.msg('请输入正整数');
					return false;
				}
				cp.minutePer = minutePer;

				if (!/^[1-9]\d*$/.test(topicNumPer)) {
					$.msg('请输入正整数');
					return false;
				}
				cp.topicNumPer = topicNumPer;

				//新增时初始化数据
				if (!cp.id) {
					cp.classTypeId = $('#classTypeId').val();
					cp.resourceType = 'TEACH_METHOD_VIDEO';
					cp.resourceId = $('.choose-course-practice-tiku').data('lecid');
				}
				...
				if (cprs.length > 1) {
					for (var _x = 0; _x < cprs.length; _x++) {
						if (!flag || !csFlag) {
							break;
						}
						cpr1 = cprs[_x];

						for (var _y = _x + 1; _y < cprs.length; _y++) {
							cpr2 = cprs[_y];
							if (cpr1.tikuCategoryId != cpr2.tikuCategoryId || cpr1.tikuSubjectId != cpr2.tikuSubjectId) {
								csFlag = false;
								break;
							}
							if (cpr1.tikuChapterId == cpr2.tikuChapterId && cpr1.tikuSectionId == cpr2.tikuSectionId) {
								flag = false;
								break;
							}
						}
					...
					if (!csFlag) {
						$.msg('第' + ++_y + '个章节练习与' + _x + '个章节练习所属题库或科目不相同');
						return;
					}
					if (!flag) {
						$.msg('第' + ++_y + '个章节练习重复');
				...
				var _video = $('.lecture .chooseVideo').data();
				if (!(_video.storageType == 'VIDEO_STORAGE_TYPE_ALIY' || _video.storageType == 'VIDEO_STORAGE_TYPE_CC' || _video.storageType == 'VIDEO_STORAGE_TYPE_QNVD' || _video.storageType == 'VIDEO_STORAGE_TYPE_BLVS' ||
						_video.storageType == 'VIDEO_STORAGE_TYPE_LETV' || _video.storageType == 'VIDEO_STORAGE_TYPE_BJY' || !_video.storageType)) {
					$.msg('您添加的文件类型不是“内部视频”，无法启用随堂练习。');
					return;
				}
				if (_video.storageType == 'VIDEO_STORAGE_TYPE_QNVD') {
					$.msg('随堂练习暂不支持七牛视频');
					return;
				}

				cp = JSON.stringify(cp);
				cprs = JSON.stringify(cprs);
				return {
					'coursePractice': cp,
					'coursePracticeRanges': cprs
				};
			} else {
				if (!cp || !('id' in cp))
					return {};
		...
		init1500: function () {
			var _html = '<div class="add-layer w1500 none" style="z-index:1001;">' +
				...
				'<select name="" id="choose_tiku_5"' +
				'style="width: 150px; height: 28px;"></select>' +
				'</span>' +
				'</div>' +
				'<div class="term-herv">' +
				'<span class="term-title">科目</span> <span class="term-content">' +
				'<select name="" id="choose_subject_5"' +
				'style="width: 150px; height: 28px;"></select>' +
				'</span>' +
				'</div>' +
				'<div class="term-herv term-herv-left">' +
				'<span class="term-title">章</span> <span class="term-content">' +
				'<select name="" id="choose_chapter_5" style="width: 150px; height: 28px;"></select>' +
				'</span>' +
				'</div>' +
				'<div class="term-herv">' +
				'<span class="term-title">节</span> <span class="term-content">' +
				'<select name="" id="choose_lecture_5" style="width: 150px; height: 28px;"></select>' +
				'</span>' +
				'</div>' +
			...
			$.ajax({
				...
				dataType: 'json',
				success: function (jsonData) {
					$("#choose_tiku_5").find("option").remove();
					$("#choose_tiku_5").append("<option value=''>选择题库</option>")
					$.each(jsonData, function (i, data) {
						$("#choose_tiku_5").append('<option value="' + data.id + '">' + data.tikuName + '</option>')
					})
				}
			})
			$(".w1500").on("change", "#choose_tiku_5", function () {
					//初始化题库下科目
					$.ajax({
						url: rootPath + '/tikuSubject/getList/' + $(this).val(),
						type: 'post',
						dataType: 'json',
						success: function (jsonData) {
							$("#choose_subject_5").find("option").remove();
							$("#choose_subject_5").append('<option value="">选择科目</option>')
							$.each(jsonData, function (i, data) {
								$("#choose_subject_5").append('<option value="' + data.id + '">' + data.subjectName + '</option>')
							})
						}
					})
				})
				.on("change", "#choose_subject_5", function () {
					//初始化题库下科目
					$.ajax({
						url: rootPath + '/tikuChapter/getList',
						data: {
						...
						dataType: 'json',
						success: function (jsonData) {
							$("#choose_chapter_5").find("option").remove();
							$("#choose_chapter_5").append('<option value="">选择章</option>')
							$.each(jsonData, function (i, data) {
								$("#choose_chapter_5").append('<option value="' + data.id + '">' + data.chapterName + '</option>')
							})
						}
					})

				})
				.on("change", "#choose_chapter_5", function () {
					//初始化题库下科目
					$.ajax({
						url: rootPath + '/tikuSection/getList',
						data: {
							tikuCategoryId: $("#choose_tiku_5").val(),
							chapterId: $(this).val()
						},
						type: 'post',
						dataType: 'json',
						success: function (jsonData) {
							$("#choose_lecture_5").find("option").remove();
							$("#choose_lecture_5").append('<option value="">选择节</option>')
							$.each(jsonData, function (i, data) {
								$("#choose_lecture_5").append('<option value="' + data.id + '">' + data.sectionName + '</option>')
							})
						}
					})
				})
				.on("click.btn", ".btn", function () {
					...
					cpr.resourceId = $('.choose-course-practice-tiku').data('lecid');
					cpr.resourceType = 'TEACH_METHOD_VIDEO';
					cpr.tikuCategoryId = $("#choose_tiku_5").find("option:selected").val();
					cpr.tikuSubjectId = $("#choose_subject_5").find("option:selected").val();
					cpr.tikuChapterId = $("#choose_chapter_5").find("option:selected").val();
					cpr.tikuSectionId = $("#choose_lecture_5").find("option:selected").val();
					cpr.tikuCategoryName = $("#choose_tiku_5").find("option:selected").val() ? $("#choose_tiku_5").find("option:selected").text() : "";
					cpr.tikuSubjectName = $("#choose_subject_5").find("option:selected").val() ? $("#choose_subject_5").find("option:selected").text() : "";
					cpr.tikuChapterName = $("#choose_chapter_5").find("option:selected").val() ? $("#choose_chapter_5").find("option:selected").text() : "";
					cpr.tikuSection = $("#choose_lecture_5").find("option:selected").val() ? $("#choose_lecture_5").find("option:selected").text() : "";

					//检查数据
					if (!cpr.tikuChapterId || !cpr.tikuSectionId) {
						$.msg("必须选择章或节");
						return;
					}
					//				_html = '<tr>'+
					//						    '<td width="30%" title="'+ cpr.tikuChapterName +'">'+ (cpr.tikuChapterName.length>10?cpr.tikuChapterName.substring(0,10):cpr.tikuChapterName) +'</td>'+
					//						    '<td width="30%" title="'+ cpr.tikuSection +'">'+ (cpr.tikuSection.length>10?cpr.tikuSection.substring(0,10):cpr.tikuSection) +'</td>'+
					//						    '<td width="40%"><a href="javascript:;" class="del-cpr">删除</a></td>'+
					//						'</tr>';
					_html = '<b>' +
						'<i class="list-item" title="' + cpr.tikuChapterName + '">' + cpr.tikuChapterName + '</i>' +
						'<i class="list-item" title="' + cpr.tikuSection + '">' + cpr.tikuSection + '</i>' +
					...
					$('.w1500 .close').trigger('click');
				}).on('click', '.close', function () {
					$('.w1500').fadeOut(200, function () {
						$(".add-layer-bg").fadeOut(200);
					});
				});
