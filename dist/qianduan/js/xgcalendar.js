...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass}" id="${id}" title="${title}" commid="${commid}" style="color:${color};"><div class="dhdV" style="display:none">${data}</div><div class="${extendClass} rb-m" style="background-color:#fe5151">${extendHTML}<div class="rb-i">${content}</div></div></div>';
        ...
        function getTitle(event) {
            ...
            var d1 = new Date(event[2]);
            var d2 = new Date(event[3]);
            eventshow = event[1];
            var startformat = getymformat(d1, null, showtime, true);
            var endformat = getymformat(d2, d1, showtime, true);
            timeshow = dateFormat.call(d1, startformat) + " - " + dateFormat.call(d2, endformat);
                ...
                for (var h = 0; h < l; ) {
                    ...
                    if (e) { //如果存在
                        ...
                        if (!bs && j == (sc - 1) && z[h] < y[h]) {
                            el++;
                            $.extend(tempdata, { "axis": h, ch: "more", "abbr": dateFormat.call(cday[h], i18n.xgcalendar.dateformat.fulldayvalue), html: i18n.xgcalendar.others, click: "javascript:alert('more event');" });
        ...
        function BuildMonthDayEvent(e, cday, length) {
            ...
            var ef = DateDiff("d", cday, new Date(e.event[3])) >= length;  //e.event[3] >= DateAdd("d", 1, cday);
            if (sf || ef) {
                if (sf) {
                    arrm.push(ml);
                ...
                $.ajax({
                    ...
                    success://function(data) {
                        ...
                        if (data != null && data.error != null) {
                            ...
                            responseData(data, new Date(data.start), new Date(data.end));
                            pushER(new Date(data.start), new Date(data.end));
                        }
        ...
        function dayshow(e, data) {
        	var url = rootPath + "/sysConfigItem/selectDetail/" 
        		+ $(this).attr("commid");
        	window.open(url);
            return false;
        }
        ...
        function moreshow(mv) {
            ...
            var ccontent = $(".alert-cont .alert-word ul");
            var ctitle = $(".alert-cont #bbit-cal-buddle-timeshow");
                ...
                for (var j = 0; j < ec; j++) {
                    ...
                    if (e) {
                        if ((e.colSpan + i - 1) >= index) {
                            htm.push("<li style='cursor: pointer' commid=" + e.event[0] + ">");
                            var start = fomartTimeShow(e.st.hour,e.st.minute,option.timeFormat);
                            var end = fomartTimeShow(e.et.hour,e.et.minute,option.timeFormat);
                            htm.push("<em>" 
                            		+ start + " - " + end +  "</em>");
                            htm.push("<i>&nbsp;&nbsp;</i><i>" + e.event[1] + "</i>");
                            htm.push("</li>");
                        }
                    }
                }
            ...
            $(".alert-cont .alert-word ul li").click(dayshow);
            return false;
        }
